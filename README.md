<!-- <img style="display: block; margin: 2rem auto; object-fit: cover; object-position: center center; width: 80vw; max-height: 10rem;" src="assets/bebop.gif" alt="Cowboy Bebop"/> -->

### Hey there 👋🏻

I'm Jason, a compsci undergrads @ [**STEI ITB**](https://www.itb.ac.id/). \
Mainly interested in **Software Engineering**, **Data Science**, and **Artificial Intelligence**.

### Touching grass

Outside technical and academic work, you'll probably find me on a hike or binge watching K-dramas ✌️

### Stats

![Stats](https://jsndwrd-github-readme-stats.vercel.app/api?username=JSNDWRD&count_private=true&theme=transparent&show_icons=true&hide_border=true)

<!-- ![Top Langs](https://jsndwrd-github-readme-stats.vercel.app/api/top-langs/?username=JSNDWRD&theme=transparent&size_weight=0.5&count_weight=0.5&layout=compact&hide_border=true) -->

### Frequently Used Tech Stacks

#### Data Structure & Algorithms

![My Skills](https://go-skill-icons.vercel.app/api/icons?i=c,cpp,haskell&perline=6)

#### Web Development

![My Skills](https://go-skill-icons.vercel.app/api/icons?i=html,css,javascript,ts,go,python,postgresql,mongodb,react,vite,nextjs,tailwindcss,express,nodejs,hono,fastapi,supabase,gcp,docker,nginx,npm,bun,drizzle,prisma&perline=6)

#### Data Science

![My Skills](https://go-skill-icons.vercel.app/api/icons?i=python,numpy,pandas,kaggle&perline=6)

### Know me more

**[LinkedIn](https://www.linkedin.com/in/jason-edward-salim/) | [Portfolio](https://jsndwrd.vercel.app/) | [Email](mailto:jasonedwardsalim.com)**
